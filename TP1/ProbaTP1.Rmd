...
**Lemma 3.3** Si Y est une v.a. de densité g et si $U ∼ Unif([0, 1])$ indépendante de Y alors : 1. la probabilité d’accepter à l’étape 3 la valeur produite à l’étape 1 de l’algorithme est : $$
...
On souhaite maintenant simuler une réalisation d’une v.a. $X ∼ N (0, 1)$. On considère la densité instrumentale suivante : pour a \> 0 $g_a(x) = \frac{a}{2} \exp(-a |x|)$ pour tout $x \in \mathbb{R}$, associée à une loi dite doublement exponentielle ou de Laplace.
...
b)  Nous acceptons un couple $(x, y)$ tiré au hasard dans le carré $[0, 1]^2$, si : $U \cdot c \cdot g(x, y) \le f(x, y)$

C’est-à-dire : \$ U \cdot \frac{1}{\text{Aire}(S)} \cdot 1\_{[0,1]\^2}(x, y) \le \frac{1}{\text{Aire}(S)} \cdot 1*S(x, y), \\ U* \cdot 1{[0,1]\^2}(x, y) \le 1\_S(x, y). \$
...
2)  
    ...
    On génère maintenant un tirage au hasard dans une surface \$S := {(x, y) ∈\mathbb{R}\^2

    :   s(x, y) ≤0}\$ (avec $s(·, ·)$ donnée) incluse dans le carré $[0, 1]^2$ . On choisit donc de simuler un cercle de centre$(0.5,0.5)$ et de rayon $0.5$.
