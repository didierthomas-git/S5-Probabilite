...
Dans cette section on a comparé les fonctions de répartition théorique et empirique pour la loi uniforme sur [0,1]. Pour un n-échantillon, on trace sa fonction de répartition empirique grâce à la fonction ecdf :
...
Une variable aléatoire $Y$ suit une loi de Laplace de paramètre $a > 0$ si elle admet pour densité $g_a(x) = \frac{a}{2} \exp(-a |x|)$ pour tout $x \in \mathbb{R}$, associée à une loi dite doublement exponentielle ou de Laplace.
...
On considère U $\sim$ Unif([0, 1]) et on cherche Y = G(U) tel que Y suive la loi de Laplace.\
...
En conclusion, Y = G(U) suit bien la loi de Laplace : la méthode de l’inverse permet de générer une valeur de cette loi à partir d’une variable uniforme U $\sim$ Unif([0, 1]).
...
**Lemma 3.3** Si Y est une v.a. de densité g et si $U \sim Unif([0, 1])$ indépendante de Y alors : 1. la probabilité d’accepter à l’étape 3 la valeur produite à l’étape 1 de l’algorithme est : $$
...
On sait que U suit une loi uniforme : $U \sim Unif([0, 1])$ et la densité de $Y$ est $g$ donc :
...
On souhaite maintenant simuler une réalisation d’une v.a. $X \sim N (0, 1)$. On considère la densité instrumentale suivante : pour a \> 0 $g_a(x) = \frac{a}{2} \exp(-a |x|)$ pour tout $x \in \mathbb{R}$, associée à une loi dite doublement exponentielle ou de Laplace.
...
On a $\forall x \in \mathbb{R}$ : $f_X (x) = \frac{1}{\sqrt{2\pi}}e^{\frac{-x^2}{2}}$, et $g_a (x) = \frac{a}{2} e^{-a|x|}$ donc $\frac{f_X (x)}{g_a (x)} = \sqrt{\frac{2}{\pi}}\frac{1}{a}e^{\frac{-x^2}{2} + a|x|}$ Cette fonction est paire. On peut donc étudier l'intervalle où $x\ge0$, le raisonnement étant similaire pour $x\le0$.

Ainsi, sur $x\ge0$ $$\frac{-x^2}{2} + a|x| = \frac{-x^2}{2} + ax$$ est un polynôme du 2nd degrès admettant un maximum pour $x=a$. Par parité, si $x\le0$, le maximum est atteint pour $x=-a$, mais la valeur du maximum demeure inchangée.
...
$$\frac{dc_a}{da} = \sqrt{\frac{2}{\pi}}(\frac{-1}{a^2}e^{\frac{-a^2}{2}}+\frac{1}{a}ae^{\frac{-a^2}{2}}) = \sqrt{\frac{2}{\pi}}e^{\frac{-a^2}{2}}(1-\frac{1}{a^2})$$ On voit que $c_a$ admet un extremum pour $a=1$. De plus si $a<1$ la fonction est décroissante, et si $a>1$, la fonction est croissante. On peut ainsi en conclure que $c_a$ est minimale pour $a=1$.
...
\mathbb{P}\{(X, Y) \in S\} = \text{Aire}\big(S \cap [0,1]^2\big)$ 1)\
a)On a : $$c \cdot g(x, y) = \frac{1}{\text{Aire}(S)} \cdot 1_{[0,1]^2}(x, y)
\ge \frac{1}{\text{Aire}(S)} \cdot 1_S(x, y)
\quad \text{puisque } S \subset [0,1]^2$$, d’où $c \cdot g(x, y) \ge f(x, y)$.

b)  Nous acceptons un couple $(x, y)$ tiré au hasard dans le carré $[0, 1]^2$, si : $U  c g(x, y) \le f(x, y)$

C’est-à-dire : $$U \cdot \frac{1}{\text{Aire}(S)} \cdot 1\_{[0,1]^2}(x, y) \le \frac{1}{\text{Aire}(S)} \cdot 1*S(x, y), \\ U* \cdot 1{[0,1]^2}(x, y) \le 1_S(x, y).$$

Si $(x, y) \in S$, alors $(x, y) \in [0,1]^2$ et l’inégalité devient $U \le 1$, ce qui est toujours vrai puisque U suit une loi uniforme.\
$\text{Ainsi, un couple } (x, y) \text{ est accepté si et seulement si } (x, y) \in S.$

2)  On génère maintenant un tirage au hasard dans une surface $$S := {(x, y) ∈\mathbb{R}^2 : s(x, y) ≤0}$$ (avec $s(·, ·)$ donnée) incluse dans le carré $[0, 1]^2$ . On choisit donc de simuler un cercle de centre$(0.5,0.5)$ et de rayon $0.5$.
...
Après modélisation, nous remarquons que la plupart des points se situent à l'intérieur de la surface du disque ce qui est cohérent car la surface du cercle représente une très grande partie de notre surface d'étude. Sur notre graphique,les points verts sont les points acceptés et les points rouges les non-acceptés. On remarque que la répartition des points est assez homogène donc bien aléatoire.
